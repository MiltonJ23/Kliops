...
// EmbeddingRequest refers to the json structure expected by ollama
...
func (o *OllamaEmbedder) CreateEmbedding(ctx context.Context, text string) ([]float32, error) {
	...
	if marshallingRequestBodyError != nil {
		return nil, fmt.Errorf("an error occurred while marshalling the request body: %v", marshallingRequestBodyError)
	}
	...
	if response.StatusCode != http.StatusOK {
		...
		return nil, fmt.Errorf("ollama sent a failed HTTP response %d: %s", response.StatusCode, string(bodyBytes))
	}
